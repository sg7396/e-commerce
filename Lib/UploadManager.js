...
function parallelUploadTOGoogleCloud(filesArray, callback) {
    //Create google cloud storage
    console.log("fyilearratyyfi",filesArray)
    var gcs=storage({
        projectId: '106154681299',
        //   keyFilename: '/var/nevermynd/neverMynd/Nevermynd-d348170f0c6d.json'
        //   keyFilename: '/home/cbl62b/nevermynd/neverMynd/Nevermynd-d348170f0c6d.json',
        keyFilename:__dirname + "/My First Project-5a5ebf110dee.json"
    })
   // console.log("gcs",gcs)
        ...
        taskToUploadInParallel.push((function (fileData) {
            return function (internalCB) {
                if (!fileData.originalPath || !fileData.nameToSave) {
                   ...
                   var bucket = gcs.bucket('swimpy');

                    gcs.bucket('swimpy').makePublic;
                    console.log(gcs,"bbbbbbbbbbbbb")
                    bucket.upload(fileData.originalPath, fileData.nameToSave, function(err, result) {
                        if (!err) {
                            console.log("file is in bucket");
...
/*function createThumbnailImage(originalPath, thumbnailPath, callback) {
    //  var gm = require('gm').subClass({imageMagick: true});
    console.log("originbalpathhhhh",originalPath)
    sharp(originalPath)
        .resize(Config.APP_CONSTANTS.SERVER.THUMB_WIDTH, Config.APP_CONSTANTS.SERVER.THUMB_HEIGHT, "!")
        .toFile(thumbnailPath, function (err, data) {
            console.log("thummnfhbhfdnbalpathhhhh",err,data)
            callback(err)
        })
}*/
